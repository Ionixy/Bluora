import { createSession, hashPassword, json, makeSessionCookie, normalizeEmail, validatePassword } from '../../_lib/auth.js';
...
export async function onRequestPost({ request, env }) {
  ...
  const result = await env.DB
    ...
    .bind(normalizedEmail, passwordHash, role, Date.now())
    .run();

  const sessionId = await createSession(env, result.meta.last_row_id);
  return json(
    { user: { id: result.meta.last_row_id, email: normalizedEmail, role } },
